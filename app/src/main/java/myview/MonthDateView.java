...
public class MonthDateView extends View{
	...
	private int mDayColor = Color.parseColor("#363636");
	...
	protected void onDraw(Canvas canvas) {
		...
		mCircleRadius =  (int) (mDaySize*mDisplayMetrics.scaledDensity*0.16);
		String dayString;
		...
		for(int day = 0;day < mMonthDays;day++){
			...
			if(dayString.equals(mSelDay+"")){
				...
				canvas.drawCircle(newCirclex, newCirley, smCircleRadius, mPaint);
				//记录第几行，即第几周
				weekRow = row + 1;
			}
			//绘制事务圆形标志
			...
			if(tv_date != null){
				//tv_date.setText(mSelYear + "." + (mSelMonth + 1) + " " + "第" + weekRow +"周");
				tv_date.setText(mSelYear + "年" + (mSelMonth + 1) + "月" + "第" + weekRow +"周");
			}
	...
	private void drawCircle(int row,int column,int day,Canvas canvas){
		if(daysHasThingList != null && daysHasThingList.size() >0){
			...
			//canvas.drawCircle(circleX, circley, smCircleRadius, mPaint);
		}
	}
	@Override
